...
**Слова для состояний — из кода самого веб-UI** (медленные чанки `static/js/1.*.js`,
найдено grep'ом по `screenStatus`/`Blackouting`): заголовок страницы рисует состояние так —

```js
n("img",{attrs:{src: t.screenStatus(t.screen)}})      // иконка: 1 -> "NIQA", 0 -> "nYE6", иначе -> "TAlh"
1 == t.screen ? "Blackouting"  : ""                   // блэкаут
2 == t.screen ? "t_Freezeing"  : ""                   // фриз
setScreenStatus: function(t,e){ … url:"/system/setScreenStatus", params:{type:t} … }
setBlackScreen()  → setScreenStatus(1) / setScreenStatus(0)   // вкл/выкл блэкаут
setFrozenScreen() → setScreenStatus(2) / setScreenStatus(0)   // вкл/выкл фриз
```

Отсюда три вывода:

1. `0` — это **картинка** (у неё нет подписи, только иконка), `1` — блэкаут, `2` — фриз. Значения
   `≥3` UI не называет (для них просто «общая» иконка) — таких состояний в штатной работе нет.
2. Оверлей **один**: `setBlackScreen`/`setFrozenScreen` держат один флаг `flag`, взаимного
   «блэкаут + фриз» у устройства не бывает.
3. **Тест-паттерн — это отдельный флаг** (`getDataBaseInputInfo` → `obj.testPicEn`, команда `Тест`
   в `commands.json`, запись через `setTestImage`). Он не входит в `screenStatus`, поэтому на
   странице статусов слова комбинируются: `картинка + тест цвет`, `блэкаут + тест цвет`, `фриз +
   тест цвет` — см. `webstatus._screen_param`; сама комбинация строится по контроллерам до
   агрегации, так что расхождение внутри группы видно как `фриз/картинка + тест цвет`.
...
Сейчас в каталоге 8 параметров (все `[verified]` по чтению, запись проверена):
Яркость, Гамма, ColorTemperature, ScreenTest (вкл/выкл), ScreenTest R/G/B, HDR.

`scal` в каталог **не входит намеренно**: это зум канваса веб-интерфейса (см.
примечание в §2.1), а не параметр картинки, и в пресете он только мешает —
попадал бы в каждый захват как «состояние», которое ничего не описывает.

`DarkMagic` и `ColorSpaceEn` из каталога **убраны** (по просьбе оператора): это
настройки обработки картинки, которые на стенде не крутят, а на странице статусов
они занимали место в кластере «Картинка». Сами эндпоинты устройства
(`setDarkMagic`/`setColorSpaceEn`) никуда не делись — они по-прежнему перечислены
в справочнике §4.1, так что вернуть параметр можно одной строкой в
`parameters.json`.
