...
3. **Config over constants-in-code.** Every tunable (ports, paths, timeouts, colors, intervals, grid size) lives in [core/config.py](core/config.py). Don't hardcode at the call site.
...
- **Magic strings.** Slot `kind`s (`"menu"`/`"command"`/`"feedback"`/`"back"`/`"prev"`/`"next"`/`"nav"`) and `ActionNode.after` (`"text"`/`"rerender"`/`"back"`) are bare strings across `model`/`layout`/`dispatcher`/`render`/`pcbrowser`. A typo fails silently — worth centralizing as constants/`Enum`.
