...
- **PixelHue poller polls even when the tab is not on screen.** `pixelhue._pull_all` fetches node detail + screens + presets every `PIXELHUE_POLL_INTERVAL` regardless of which page is visible (3 small requests per cycle; fine for one device, but it could be scoped to skip when no page sits in the PixelHue subtree).
