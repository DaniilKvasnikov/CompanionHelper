...
- **PDQ deployment journal on the page** — the status page's **PDQ** panel lists what PDQ Deploy is running and how the recent tasks ended (package, status, how many PCs, start/finish; per-target detail, plus every raw column, in the tooltip). On-prem PDQ Deploy has no REST API, so this reads its SQLite database directly ([`core/pdq.py`](core/pdq.py)); if the database, the table or a column is not where the reader expects it, the log window names what the database actually contains instead of just showing nothing. Console equivalent: `python -m core.pdq deployments`.
- **Web status page** — open `http://<this machine>:7878/` in a browser to watch the live state of everything the deck can turn: every Aoto parameter of every group (brightness in nits *and* % of the group's `@max` ceiling, HDR, gamma, colour temperature, **what is on the screen in words** — `картинка` / `блэкаут` / `фриз`, with `+ тест цвет` while the colour test pattern is on — …), **grouped by the device's own sections** (Картинка / Вход и тест-паттерн / Система) as a grid of values, the **PDQ journal** of running and recent deployments, the ping status of the PCs in the active PDQ list, and the PixelHue node/flags/screens/presets. It is **read-only** — nothing on it writes to a device or to the deck — and it is built to fit **one screen without scrolling**: four panels in a 2×2 grid, and the page shrinks its own font if the content would overflow (details that don't fit live in tooltips). Each Aoto group's headline reads as the deck's `OK n/n` — **how many of the group's controllers answered** (`1/2 ок` for a two-controller group with one down), not a count of parameters. The page polls the server once a second, and while a tab is open the server re-reads the devices every `WEB_REFRESH_INTERVAL` (2 s) and shows how old the data is; close the tab and the extra polling stops (`WEB_IDLE_TIMEOUT`). The **Логи** button opens a separate window that explains **why** something is not being read — the exact address/URL tried, what came back (`timed out`, `HTTP 404`, «в ответе нет поля obj.brightness», «в файле … нет адресов контроллеров», «не читается база PDQ») and how often — so "aoto не в сети" becomes a concrete, fixable reason. Rows keep their order, **a click copies the line** to the clipboard for pasting, and **Пауза** freezes the list when it changes too fast to read.
...
TOUCH_API_GUIDE.md     the API TouchDesigner talks to (register buttons, ping, OSC press back)
tests/             pytest suite
```
